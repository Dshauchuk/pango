...
internal static class PangoExplorerItemExtensions
{
    ...
    {
        item.CatalogPath = item.Parent is null
            ? string.Empty
            : $"{(string.IsNullOrEmpty(item.Parent.CatalogPath) ? string.Empty : item.Parent.CatalogPath + AppConstants.CatalogDelimeter)}{item.Parent.Name}";
